...
if __name__ == '__main__':
    ...
    for n in range(10000):
        ...
        xb, yb = jloaders[loader].sample_batch(bsize = batch_size, sequence_len = context_len, print_sample = False)
        
        x = torch.from_numpy(xb).view((batch_size, context_len, C, D, D)).to(device)
        xl = get_latents(state_encoder, x, context_len, latent_size).to(device)

        #print("got latents ", xl.shape)
        #print("got y ",yb.shape)
